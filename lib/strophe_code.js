...
////        var t = setTimeout("ask_for_friendship()",5000);
        //trigger it instead when name aked for
      }
...
//triggered when we have the name from the overlay, I hope
    ...
    $.each(roster.contacts, function (jid) {
        ...
        if(status=="available"){
          ...
          if(name.match("telly") || name.match("tv")){ // shoudl be groups!! although there's only 1 tv ever atm (the brain)
            ...
            html.push("<div class='center'><img src='images/tv.png' height='100' /></div>");
